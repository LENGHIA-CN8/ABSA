...
def main():
    ...
    # Log
    if not os.path.isdir("../log/FandB/"+ args.task):
        os.mkdir("../log/FandB/"+ args.task)
    log_file = "../log/FandB/" + args.task + "/" + args.approach
    for key, val in list_args:
    ...
    # Save model
    if not os.path.isdir("../saved_model/FandB/" + args.task):
        os.mkdir("../saved_model/FandB/" + args.task)
    save_model_path = log_file.replace("log", "saved_model").replace(".txt", ".pt")
    torch.save(trainer.model.state_dict(), save_model_path)

    # Save result
    import csv
    if not os.path.isdir("../result/FandB/" + args.task):
        os.mkdir("../result/FandB/" + args.task)
        
    save_result_path = os.path.join("../result/FandB/", args.task, args.approach + ".csv")
