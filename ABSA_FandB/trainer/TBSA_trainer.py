...
class Trainer():
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            for step, batch in enumerate(self.train_loader):
                ...
                loss, logits = self.model(input_ids, attention_mask, target_masks, labels, self.args.data_mode, self.args.pool_type, self.label_weights)
            ...
            valid_loss, valid_acc, valid_micro_f1, valid_macro_f1, valid_confusion = self.eval(self.model, self.valid_loader)
            train_loss, train_acc, _, _, _ =  self.eval(self.model, self.train_loader)
            ...
            print("Epoch {:3d} | Train: loss= {:.3f}, acc= {:.3f}% || Valid: loss= {:.3f}, acc= {:.3f}%, micro_f1= {:.3f}%, macro_f1= {:.3f}% || Best macro_f1: {:.3f} || Time= {:.3f}s".format(epoch + 1, train_loss, train_acc*100, valid_loss, valid_acc*100, valid_micro_f1*100, valid_macro_f1*100, self.best_valid_macro_f1*100, time2- time1), file= self.log_file)
    
            print("Epoch {:3d} | Train: loss= {:.3f}, acc= {:.3f}% || ".format(epoch + 1, train_loss, train_acc), end= "")
            print("Valid: loss= {:.3f}, acc= {:.3f}%, micro_f1= {:.3f}%, macro_f1= {:.3f}% || Best macro_f1: {:.3f} || Time= {:.3f}s ||".format(valid_loss, valid_acc*100, valid_micro_f1*100, valid_macro_f1*100, self.best_valid_macro_f1*100, time2- time1), end= "\n")
    ...
    def eval(self, model, data):
        ...
        for step, batch in enumerate(data):
            ...
            batch = [t.to(self.device) for t in batch]
            input_ids, attention_mask, token_type_ids, target_masks, labels = batch
            loss, logits = model(input_ids, attention_mask, target_masks, labels, self.args.data_mode, self.args.pool_type, self.label_weights)
        ...
        confusion_matrix = get_confusion_matrix(groundtruths, prediction, self.args.label_dict)
        return loss, acc, micro, macro, confusion_matrix
