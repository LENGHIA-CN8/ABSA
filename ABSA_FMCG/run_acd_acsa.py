...
args.label_weights = label_weights_acd_acsa
if args.imb_weight:
...
if args.approach == "phobert":
    ...
    keys_in_name = ["seed", "num_classes", "max_length", "stride", "lr", "epochs", "imb_weight", "data_mode", "pool_type", "weight_decay", "do_lower_case", "replace_term_with_mask", "train_batch_size", "test_batch_size"]
    ...
    MODEL_CLASSES[args.approach] = (RobertaConfig, Net, PhobertTokenizer)
    keys_in_name = ["seed", "num_classes", "max_length", "stride", "mix_type", "mix_count", "lr", "epochs", "imb_weight", "data_mode", "pool_type", "weight_decay", "do_lower_case", "replace_term_with_mask", "train_batch_size", "test_batch_size"]
...
def main():
    ...
    DataLoader = Loader(args, args.aspect_type_dict, args.aspect_sentiment_dict, tokenizer)
    ...
    time_test1 = time()
    test_loss, test_acc, test_micro_f1, test_macro_f1, test_confusion, test_strict_acc_acd, test_strict_acc_acd_acsa = trainer.eval(trainer.model, trainer.test_loader)
    time_test2 = time()
    ...
    print("Test: loss= {:.3f}, acc= {:.3f}%, mirco_f1= {:.3f}%, macro_f1= {:.3f}%, strict_acc_acd= {:.3f}%, strict_acc_acd_acsa= {:.3f}%".format(test_loss, test_acc*100, test_micro_f1*100, test_macro_f1*100, test_strict_acc_acd*100, test_strict_acc_acd_acsa*100))
    print("Infer time: {:.3f}ms".format(1000* (time_test2- time_test1)/ (args.test_batch_size*len(test_dataloader))))
    print("Test: loss= {:.3f}, acc= {:.3f}%, mirco_f1= {:.3f}%, macro_f1= {:.3f}%, strict_acc_acd= {:.3f}%, strict_acc_acd_acsa= {:.3f}%".format(test_loss, test_acc*100, test_micro_f1*100, test_macro_f1*100, test_strict_acc_acd*100, test_strict_acc_acd_acsa*100), file= f_log)
    ...
    if not os.path.isfile(save_result_path):
        with open(save_result_path, mode='a') as f:
            ...
            writer.writerow([key for key, val in list_args if key in keys_in_name] + ["test_acc", "test_micro_f1", "test_macro_f1", "test_strict_acc_acd", "test_strict_acc_acd_acsca", "valid_macro_f1"])
            writer.writerow([val for key, val in list_args if key in keys_in_name] + ["{:.3f}".format(test_acc*100), "{:.3f}".format(test_micro_f1*100), "{:.3f}".format(test_macro_f1*100), "{:.3f}".format(test_strict_acc_acd*100), "{:.3f}".format(test_strict_acc_acd_acsa*100), "{:.3f}".format(trainer.best_valid_macro_f1*100)])
    else:
        with open(save_result_path, mode='a') as f:
            writer = csv.writer(f, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            writer.writerow([val for key, val in list_args if key in keys_in_name] + ["{:.3f}".format(test_acc*100), "{:.3f}".format(test_micro_f1*100), "{:.3f}".format(test_macro_f1*100), "{:.3f}".format(test_strict_acc_acd*100), "{:.3f}".format(test_strict_acc_acd_acsa*100), "{:.3f}".format(trainer.best_valid_macro_f1*100)])
